...
if ( in_array('plugins', $show_pages) ) {
	$plugins = sprintf(
		( ! $theme_active ) ? '<li class="mtips inactive"><a href="%s" class="et-nav%s et-nav-general">%s</a><span class="mt-mes">' . $mtips_notify . '</span></li>' : '<li><a href="%s" class="et-nav%s et-nav-general">%s</a></li>',
...
if ( in_array('patcher', $show_pages) ) {
    $plugins .= sprintf(
        ( ! $theme_active ) ? '<li class="mtips inactive"><a href="%s" class="et-nav%s et-nav-css">%s</a><span class="mt-mes">' . $mtips_notify . '</span></li>' : '<li><a href="%s" class="et-nav%s et-nav-css">%s</a></li>',
        ...
        esc_html__( 'Patcher', 'xstore' ) . $new_label
    );
}
...
if ( in_array('system_requirements', $show_pages) ) {
	$system_requirements = sprintf(
		'<li><a href="%s" class="et-nav%s et-nav-page">%s</a></li>',
...
//				'🚀&nbsp;&nbsp;' . esc_html__( 'Sales Booster', 'xstore' ) . $new_label
		...
		//'<li class="mtips"><a href="%s" class="et-nav%s et-nav-branding">⚡ %s <span style="margin-left: 5px; background: var(--et_admin_green-color, #489c33); letter-spacing: 1px; font-weight: 400; display: inline-block; text-transform: lowercase; border-radius: 3px; color: #fff; padding: 3px 2px 2px 3px; text-transform: uppercase; font-size: 8px; line-height: 1;">new</span>'.
			'<li'.(!$amp_active ? ' class="mtips"' : '').'><a href="%s" class="et-nav%s et-nav-branding"> %s <span style="margin-left: 5px; background: var(--et_admin_green-color, #489c33); letter-spacing: 1px; font-weight: 400; display: inline-block; text-transform: lowercase; border-radius: 3px; color: #fff; padding: 3px 2px 2px 3px; text-transform: uppercase; font-size: 8px; line-height: 1;">new</span>' .
            (!$amp_active ? '<span class="mt-mes">' . esc_html__( 'Install and Activate XStore AMP plugin to use amp settings', 'xstore' ) . '</span>' : '') . '</a></li>',
...
if ( in_array('open_ai', $show_pages) ) {
    $out .= sprintf(
        (!$core_active || !$theme_active) ? '<li class="mtips inactive"><a href="%s" class="et-nav%s et-nav-css">%s</a><span class="mt-mes">' . $mtips_notify . '</span></li>' : '<li><a href="%s" class="et-nav%s et-nav-css">%s</a></li>',
        ($theme_active && $core_active) ? admin_url('admin.php?page=et-panel-ai') : admin_url('admin.php?page=et-panel-welcome'),
        ($_GET['page'] == 'et-panel-ai') ? ' active' : '',
        esc_html__('ChatGPT (OpenAI)', 'xstore') . $new_label
    );
}
