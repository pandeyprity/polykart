    ...
    <?php if($connection) : ?>
        ...
        <?php if ( in_array('reset', $_POST['steps']) ): ?>
            <div class="et_popup-step et_step-reset <?php echo esc_attr($classes['et_step-reset']); ?>">
                ...
                <ul class="text-left">
                    <li>1. <?php echo sprintf(esc_html__('To ensure optimal performance, it is recommended to run the import process on a fresh WordPress installation. The %1$1sAdvanced WordPress Reset%2$2s plugin can be used for this purpose.', 'xstore'), '<a href="https://wordpress.org/plugins/advanced-wp-reset/" target="_blank" rel="nofollow" style="color: #c62828;">', '</a>'); ?></li>
                    <li>2. <?php echo esc_html__('Please note that importing a site will not delete any existing pages or posts, but it may overwrite existing content.', 'xstore'); ?></li>
                    <li>3. <?php echo esc_html__('As per copyright laws, any copyrighted media will not be imported and will be replaced with placeholders.', 'xstore'); ?></li>
                </ul>
            </div>
        <?php endif; ?>
        ...
        <?php if ( in_array('child_theme', $_POST['steps']) ): ?>
            ...
            <div class="et_popup-step et_step-child_theme text-left <?php echo esc_attr($classes['et_step-child_theme']); ?>">
                ...
                <p class="step-description">
                    <?php esc_html_e('It is highly recommended to use a child theme. This will enable the parent theme to receive updates without overwriting any of your custom source code changes.', 'xstore') ?>
                </p>
