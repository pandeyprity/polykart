...
//                $new_total += $values['data']->get_price() * $quantity;
    ...
    public function get_cart_subtotal_filter( $subtotal ) {
        ...
        foreach ( $items as $item => $values ) {
            ...
            $quantity     = $values['quantity'];
            if ( isset($values['et_discount_price']))
                $new_subtotal += $values['et_discount_price'] * $quantity;
            else
                $new_subtotal += $values['data']->get_price() * $quantity;
        }
...
//            }
    ...
    public function calculate_price_discounts_filter($price, $cart_item, $cart_item_key) {
        if ( isset($cart_item['et_discount_price']) )
            return wc_price($cart_item['et_discount_price']);
        return $price;
    }
        ...
        if ( isset($cart_item['et_discount_price']) ) {
            if ( $cart_item['data']->is_taxable() ) {
                ...
                if ( WC()->cart->display_prices_including_tax() ) {
                    $row_price        = wc_get_price_including_tax( $cart_item['data'], array( 'qty' => $cart_item['quantity'], 'price' => $cart_item['et_discount_price'] ) );
                    ...
                    $row_price        = wc_get_price_excluding_tax( $cart_item['data'], array( 'qty' => $cart_item['quantity'], 'price' => $cart_item['et_discount_price'] ) );
                    $product_subtotal = wc_price( $row_price );
                ...
                $row_price        = (float) $cart_item['et_discount_price'] * (float) $cart_item['quantity'];
                $product_subtotal = wc_price( $row_price );
            }
        }
            ...
            <div class="quantity-discount-item">
                <div class="quantity-discount-info">
                    ...
                    if ( $settings['rules'] == 'intervals')
                        echo sprintf(__('%1s Buy from %2s to %3s items and get %4s OFF %5s %6s on each product %7s', 'xstore'), '<h5 class="quantity-discount-name">', $discount['min'], ($discount['max'] ? $discount['max'] : '&#8734;'), $discount['percentage'].
                        ($settings['type'] == 'percentage' ? '%' : $currency_symbol ),
                        '</h5>', '<span class="quantity-discount-suggest">', '</span>');
                    else
                        echo sprintf(__('%1s Buy %2s items get %3s OFF %4s %5s on each product %6s', 'xstore'), '<h5 class="quantity-discount-name">', $discount['every'], $discount['percentage'].
                            ($settings['type'] == 'percentage' ? '%' : $currency_symbol ), '</h5>', '<span class="quantity-discount-suggest">', '</span>');
                    ?>
                </div>
                <div class="quantity-discount-add">
                    ...
                    <button type="submit" class="add_to_cart_button ajax_add_to_cart button btn bordered"
                        ...
                        <?php
                        if ( $settings['button_icon'] != 'none' )
                            echo '<span class="et_b-icon et-icon '.str_replace('et_icon-', 'et-', $settings['button_icon'] ).'"></span>';
                        if ( $settings['button_text'] !== false )
                            echo '<span>' . ($settings['button_text'] ? $settings['button_text'] : esc_html__('Add', 'xstore')) . '</span>';
                        ?>
                    </button>
                </div>
            </div>
            ...
            <div class="options_group">
                <p class="form-field">
                    ...
                    woocommerce_wp_radio([
                            ...
                            'options' => [
                                'percentage' => sprintf(__('By percentage (example: 10%1s OFF)', 'xstore'), '%'),
                                'fixed' => sprintf(esc_html__( 'By fixed price (example: 10%1s OFF)', 'xstore' ), get_woocommerce_currency_symbol( get_woocommerce_currency() )),
                            ],
                        ]
                    );
