...
//            if ( version_compare( $value['version'], $theme_version, '<' ) ) continue;
            if ( !$test_mode && $value['status'] == 'inactive' ) continue;
    ...
    public function get_patches_list($theme_version = '1.0.0')
    {
        ...
        if (!count($patches)) {
            echo '<p class="et-message et-info">' . sprintf(esc_html__('Currently, you have installed the most recent version. There are no updates or patches available at this time. In the unlikely event that you encounter any bugs, please reach out to our support team through the provided %slink%s and we will work to resolve the issue promptly.', 'xstore'), '<a href="https://www.8theme.com/forums/xstore-wordpress-support-forum/" rel="nofollow" target="_blank">', '</a>') . '</p>';
            ?>
            <p>
                <span class="et-button et-button-dark-grey refresh-patches"
                      data-nonce="<?php echo wp_create_nonce('refresh-patches'); ?>">
                    ...
                    <?php esc_html_e('Check for updates', 'xstore'); ?>
                </span>
            </p>
            <?php
            return;
        ...
        echo '<p>' .
            sprintf(esc_html__('Unleash the full potential of your website with The Parcher! With its unique ability to apply small fixes between XStore releases, you can now keep your site up-to-date and running smoothly. In the unlikely event that you encounter any bugs, please reach out to our support team through the provided %slink%s and we will work to resolve the issue promptly.', 'xstore'),
                '<a href="https://www.8theme.com/forums/xstore-wordpress-support-forum/" rel="nofollow" target="_blank">', '</a>') .
            '</p>';
        ...
        <p>
            <span class="et-button et-button-dark-grey refresh-patches"
                ...
                <span class="dashicons dashicons-image-rotate"></span>
                <?php esc_html_e('Refresh patches', 'xstore'); ?>
            </span>
        </p>
        <div class="etheme-patcher-table-wrapper">
            <table>
                ...
                foreach ($this->patches_info as $patch_info) {
                    ...
                    $description_class = $description_tooltip ? ' mtips mtips-lg' : '';
    ...
    public function get_patches_info($patches, $on_test = false)
    {
        ...
        $all_applied_patches_keys = !empty($applied_patches) ? array_merge(...array_values($applied_patches)) : $applied_patches;
    ...
    public function get_patch_file_from_server($patch_id, $file_id)
    {
        ...
        if (!isset($response['content'])) {
            return array(
                'message' => esc_html__('Error', 'xstore') . ': ' . sprintf(esc_html__('We would like to inform you that the file you have downloaded has no content. Please kindly inform us via our contact page %1s so that we can fix the issue immediately.', 'xstore'), 'https://www.8theme.com/contact-us/'),
                'status' => 'error',
            );
        }
        ...
        if ($multiple) {
            ...
            echo wp_json_encode(
                array(
                    ...
                    'errors_text' => __('Some files were unable to be modified on your server. Please check your "Server Requirements" page or try again later.', 'xstore'),
    ...
    public function apply_all_patches()
    {
        ...
        echo wp_json_encode(
            array(
                ...
                'errors_text' => __('We apologize for the inconvenience, but it seems that some patches could not be applied. Please make sure to check your "Server Requirements" page or try again later. Thank you.', 'xstore'),
                'system_requirements_link' => add_query_arg(array('page' => 'et-panel-system-requirements'), admin_url('admin.php')),
