					...
					foreach( $terms as $term ) {
						...
						if ( in_array( $term->slug, $variations, true ) ) {
							...
							$html .= sprintf(
								...
								esc_attr( $term->slug ),
								esc_attr( $term->name ),
								( $image ) ? wp_get_attachment_image( $image, apply_filters('sten_wc_single_image_swatch_size', 'thumbnail') ) : '<img src="'.esc_url( $this->plugin_public_dir_path_url . 'images/placeholder.png' ).'"/>',
								esc_attr( $subtype )
							);
						}
					}
