					...
					foreach( $terms as $term ) {
						...
						if ( in_array( $term->slug, $variations, true ) ) {
							...
							$out .= sprintf(
								...
								esc_attr( $term->slug ),
								esc_attr( $term->name ),
								( $image ) ? wp_get_attachment_image( $image, apply_filters('sten_wc_archive_loop_image_swatch_size', 'thumbnail') ) : '<img src="'.esc_url( ET_CORE_URL . 'packages/st-woo-swatches/public/images/placeholder.png' ) . '"/>' ,
								esc_attr( $subtype )
							);
						}
					}
