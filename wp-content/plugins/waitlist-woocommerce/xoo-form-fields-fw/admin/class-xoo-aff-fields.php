	...
	*/
	...
	public function get_fields_layout( $args = array() ){
		...
		foreach ( $fields as $field_id => $field_data ){
			$html .= $this->get_field_html( $field_id, array( 'return' => true ) );
		}
	...
	public function save_settings(){
		...
		if( !isset( $_POST['submit_nonce'] ) || !wp_verify_nonce( $_POST['submit_nonce'], 'xoo-aff-submit-nonce' ) || !current_user_can( $this->aff->admin->capability ) ){
			wp_die( 'Cheating.' );
		}

		if( !$this->aff->is_fields_page_ajax_request() ) return;

		$fields = json_decode( stripslashes( wp_kses_post( $_POST['xoo_aff_data'] ) ), true);
	...
	*/
	public function get_input_html( $field_id, $args ){
		...
		if( $args['return'] ){
			...
			echo wp_kses( $field_html, xoo_wlext() );
		}
	...
	*/
	...
	public function validate_submitted_field_values( $values = array(), $do_not_validate_ids = array() ){
		...
		foreach ( $fields as $field_id => $field_data ) {
			...
			//Field Validation
			$userVal 	= isset( $values[ $field_id ] ) ? ( is_array( $_POST[ $field_id ] ) ? array_map( 'sanitize_text_field', $_POST[ $field_id ] ) : sanitize_text_field( trim( $values[ $field_id ] ) ) ) : '';
			...
			if( $settings['required'] === "yes" &&  !$userVal ){
				...
				switch ( $field_data['input_type'] ) {
					case 'checkbox_single':
						$errors->add( 'not-checked', sprintf( esc_html__( 'Please check %s.', 'easy-login-woocommerce' ), $label ), $field_id );
						break;
					
					default:
						$errors->add( 'empty', sprintf( esc_html__( '%s cannot be empty.', 'easy-login-woocommerce' ), $label ), $field_id );
						break;
				}	
			...
			if( isset( $settings['minlength'] ) && !empty( $settings['minlength'] ) && strlen( $userVal ) < (int) $settings['minlength']  ){
				$errors->add( 'minlen', sprintf( esc_html__( '%s needs to be minimum %s characters.', 'easy-login-woocommerce' ), $label, $settings['minlength'] ), $field_id );
			}

			//Check max characters
			if( isset( $settings['maxlength'] ) && !empty( $settings['maxlength'] ) && strlen( $userVal ) > (int) $settings['maxlength']  ){
				$errors->add( 'maxlen', sprintf( esc_html__( '%s cannot exceed %s characters.', 'easy-login-woocommerce' ), $label, $settings['maxlength'] ), $field_id );
			}

			//Check min value
			if( isset( $settings['min'] ) && !empty( $settings['min'] ) && ( float ) $userVal < (float) $settings['min']  ){
				$errors->add( 'min', sprintf( esc_html__( '%s should be minimum %s.', 'easy-login-woocommerce' ), $label, $settings['min'] ), $field_id );
			}

			//Check max value
			if( isset( $settings['max'] ) && !empty( $settings['max'] ) && ( float ) $userVal > (float) $settings['max']  ){
				$errors->add( 'max', sprintf( esc_html__( '%s cannot be more than %s.', 'easy-login-woocommerce' ), $label, $settings['max'] ), $field_id );
			}

			//Check Step
			if( isset( $settings['step'] ) && $settings['step'] !== 'any' && ( (float) $userVal % (float) $settings['step'] ) !== 0 ){
				$errors->add( 'step', sprintf( esc_html__( '%s should be in multiple of %s.', 'easy-login-woocommerce' ), $label, $settings['step'] ), $field_id );
			}
